...
$(document).ready(function () {
    ...
    $(".form-cart-group").on('submit',function(e){
        e.preventDefault();

        $(".form-cart-group").find(".help-block").remove();
        ...
        $(this).find('.data').each(function () {
          ...
          arr.pushArray([
            ...
            obj.final_price = $(this).find("input[name='final_price']").val();
        ...
        $.ajax({
            type: "POST",
            url: window.location.origin+'/add-to-cart-group',
            dataType: 'json',
            data: ,
            success: function( data ) {
                // swal(data.message, "success");
                if(data.success === false){
                    $(".form-cart-group").append("<span class='help-block'>"+data.message+"</span>");
                }else{
                    disPlayMessage(data);
